...
#if (MALI_DVFS_STEPS > 4)
	{ 1150000, 1137500, 1125000, 1112500, 1100000, 1087500, 1075000, 1062500, 1075000, 1062500, 1037500, 1025000, 1012500},	/* L0(600Mhz) */
#endif
#endif
#endif
#endif
...
int change_dvfs_tableset(int change_clk, int change_step)
{
	...
	if (maliDvfsStatus.currentStep == change_step) {
		...
		/* lock/unlock CPU freq by Mali */
		if (mali_dvfs[change_step].clock >= 600)
			err = cpufreq_lock_by_mali(1400);
